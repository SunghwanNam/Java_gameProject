...
public class GameManager {
	public void playGame() {
		...
		// 게임을 시작할때에 선공후공을 선택하기 위한 코드가 GameManager에 너무 몰려 셋팅값은 GameSystem이라는 클래스를 만들어
		// 객체로 만들었다.
...
//			int team1Index = random.nextInt(team1.length);
			if (gs.whosFisrt == 0) {
				if (team0[team0Index].hp > 0 || team1[team1Index].hp > 0) {
					team0[team0Index].underAttack(team0[team0Index].getPower());
					team1[team1Index].underAttack(team1[team1Index].getPower());
				} else {
					continue;
				}
			}
			...
			// team0IsAlive의 초기값이 false이기 때문에 isLive의 객체가 true이면 원래의 값이 true로 나오게 된다.
			// 둘다 false일때에 false값으로 떨어지게 된다.
